...
public class BizTraceResolver extends AbstractResolver {
	 ...
	 * @param log
	 */
	private void doRedisSave(BizTraceLogRecord log) {
		...
		if (!StringUtils.isEmpty(log.serialNo) && !"null".equals(log.serialNo)) {
			 ...
			 * #按日期存储交易流水号集合
			 */
			redisClientTemplate.sadd(String.format(RunConfig.KP_SET_SERIALNOS, log.date), log.serialNo);// 用set保存date日的交易流水号
			// redisClientTemplate.sadd(String.format(RunConfig.KP_SET_SERIALNOS,
			// log.date), log.serialNo) ;
			...
			redisClientTemplate.hset((String.format(RunConfig.KP_MAP_SERIALNO, log.serialNo)).getBytes(),
					(log.time + "-" + log.uuid).getBytes(), logBytes);
			// redisClientTemplate.hset((String.format(RunConfig.KP_MAP_SERIALNO,
			// log.serialNo)).getBytes(),
			// (log.time+"-"+log.uuid).getBytes(),
			// logBytes) ;
		}

		/*
		 * 如果该日志记录有流水号则： #存储有流水号关联的uuid 否则：#存储无流水号关联的uuid
		 */
		if (StringUtils.isEmpty(log.serialNo)) {

			redisClientTemplate.sadd(String.format(RunConfig.KP_SET_UNLINK_UUID, log.date), log.uuid);
		} else {

			redisClientTemplate.sadd(String.format(RunConfig.KP_SET_LINK_UUID, log.date), log.uuid);
		}
		...
		if (!StringUtils.isEmpty(log.type) && !"null".equals(log.type)) {
			...
			redisClientTemplate.sadd(String.format(RunConfig.KP_SET_REQUEST_TYPE, log.date, log.type),
					log.type + ":" + log.uuid);
		}
