...
public class OmGroupRServicelmpl extends BaseRService implements IGroupRService {
     ...
     * </pre>
     *
     * @param groupType 工作组类型（值来自业务菜单： DICT_OM_GROUPTYPE）
     * @return 工作组代码
...
//		map.put("groupType", groupType);
        return boshGenGroupCode.genGroupCode(groupType);
    }
     ...
     */
    @Override
    public OmGroup createRootGroup(OmGroup group) throws GroupManagementException {
        if (group == null) {
            throw new GroupManagementException(ExceptionCodes.NOT_ALLOW_NULL_WHEN_CALL, wrap("createRootGroup"));
        }
        if (StringUtils.isBlank(group.getGroupType())) {
            throw new GroupManagementException(ExceptionCodes.LACK_PARAMETERS_WHEN_INSERT, wrap(OmGroup.COLUMN_GROUP_TYPE, OmGroup.TABLE_NAME));
        }
        // 补充信息
        group.setGuid(GUID.group());// 补充GUID
        group.setGroupCode(boshGenGroupCode.genGroupCode(group.getGroupType()));
     ...
     * </pre>
     *
     * @param groupType       新工作组类型
     * @param groupName       新工作组名称
     * @param orgCode         隶属机构代码
     * @param parentGroupCode 父工作组代码
     * @return 新增的工作组对象
     * @throws GroupManagementException
     */
    @Override
    public OmGroup createGroup(String groupType, String groupName, String orgCode,
                               String parentGroupCode) throws GroupManagementException {
        //验证传入参数
        if (StringUtil.isEmpty(groupType)) {
            throw new GroupManagementException(OMExceptionCodes.LAKE_PARMS_FOR_CREATE_GROUP, wrap(groupType));
        }
        if (StringUtil.isEmpty(groupName)) {
            throw new GroupManagementException(OMExceptionCodes.LAKE_PARMS_FOR_CREATE_GROUP, wrap(groupName));
        }
        if (StringUtil.isEmpty(orgCode)) {
            throw new GroupManagementException(OMExceptionCodes.LAKE_PARMS_FOR_CREATE_GROUP, wrap(orgCode));
        }
        //转换orgCode为orgGuid
        ...
        if (orgList.size() != 1) {
            throw new GroupManagementException(OMExceptionCodes.ORGANIZATION_NOT_EXIST_BY_ORG_CODE, wrap(orgCode));
        }
        ...
        if (StringUtil.isEmpty(parentGroupCode)) {//为空新建根工作组
            ...
            og.setGroupCode(boshGenGroupCode.genGroupCode(groupType));
            ...
            if (ogList.size() != 1) {
                throw new GroupManagementException(OMExceptionCodes.GROUP_NOT_EXIST_BY_GROUP_CODE, wrap(parentGroupCode));
            }
    ...
    @Override
    public List<OmGroup> queryAllGroup() throws GroupManagementException {
     ...
     */
    @Override
    public OmGroup createGroup(OmGroup group) throws GroupManagementException {
        if (group == null) {
            throw new GroupManagementException(ExceptionCodes.NOT_ALLOW_NULL_WHEN_CALL, wrap("createGroup"));
        }
        if (StringUtils.isBlank(group.getGroupType())) {
            throw new GroupManagementException(ExceptionCodes.LACK_PARAMETERS_WHEN_INSERT, wrap(OmGroup.COLUMN_GROUP_TYPE, OmGroup.TABLE_NAME));
        }
    ...
    @Override
    public OmGroup copyGroup(String fromGroupCode, String toOrgCode, String toParentGroupCode)
            throws GroupManagementException {
    ...
    @Override
    public OmGroup copyGroupDeep(String fromGroupCode, String toOrgCode, String toParentGroupCode,
    ...
    @Override
    public OmGroup updateGroup(OmGroup newOmGroup) throws GroupManagementException {
        WhereCondition wc = new WhereCondition();
        wc.andEquals("GROUP_CODE", newOmGroup.getGroupCode());
        List<OmGroup> ogList = omGroupService.query(wc);
        if (ogList.size() != 1) {
            throw new GroupManagementException(OMExceptionCodes.GROUP_NOT_EXIST_BY_GROUP_CODE, wrap(newOmGroup.getGroupCode()));
        }
    ...
    @Override
    public OmGroup deleteGroup(String groupCode) throws GroupManagementException {
        //校验入参
    ...
    @Override
    public OmGroup cancelGroup(String groupCode) throws GroupManagementException {
        OmGroup og = queryGroup(groupCode);
    ...
    @Override
    public OmGroup reenableGroup(String groupCode, boolean reenableChile) throws GroupManagementException {
        //调用方法中已经有参数检验
        OmGroup og = queryGroup(groupCode);
        List<OmGroup> ogList = queryAllchild(groupCode);
        if (reenableChile) {
    ...
    public OmGroup queryGroup(String groupCode) {
        ...
        List<OmGroup> ogList = omGroupService.query(wc);
        if (ogList.size() != 1) {
            throw new GroupManagementException(OMExceptionCodes.GROUP_NOT_EXIST_BY_GROUP_CODE, wrap(groupCode));
        }
        OmGroup og = ogList.get(0);
        return og;
    }
    ...
    public List<OmGroup> queryBygroupName(String groupName) {
        if (StringUtil.isEmpty(groupName)) {
            throw new GroupManagementException(OMExceptionCodes.PARMS_NOT_ALLOW_EMPTY, wrap("groupName"));
        }
        WhereCondition wc = new WhereCondition();
    ...
    public void addGroupApp(String appGuid, String groupGuid) {
        ...
        if (StringUtil.isEmpty(groupGuid)) {
            throw new OrgManagementException(OMExceptionCodes.PARMS_NOT_ALLOW_EMPTY, wrap("groupGuid"));
        }
    ...
    public List<AcApp> queryApp(String groupCode) {
        // 校验传入参数
        if (StringUtil.isEmpty(groupCode)) {
            throw new OrgManagementException(OMExceptionCodes.PARMS_NOT_ALLOW_EMPTY, wrap("groupCode"));
        }
        WhereCondition wc = new WhereCondition();
        OmGroup og = queryGroup(groupCode);
