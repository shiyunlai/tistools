...
(function  ($) {
	$.fn.dargFlex = function  (classN,json) {
		...
		json.dragStyle.zIndex = json.dragStyle.zIndex  || 9999;
		...
		this.bind('mousedown',function (ev){
			//console.log('dddd')
			var oEe = ev || event;
			var oSrc =  oEe.target.parentNode;			//默认不带参数，默认全部子项拖拽

			//console.log(oEe.target)
			if ($(oEe.target.parentNode).hasClass(classN)){		//默认以类名匹配拖拽元素
				//console.log($this[0])
				var oldY = oEe.clientY;
				var oldX = oEe.clientX;
				var iTop = oEe.target.parentNode.offsetTop;
				var iLeft = oEe.target.parentNode.offsetLeft;
				var iW  = oEe.target.parentNode.offsetWidth;
				var iH = oEe.target.parentNode.offsetHeight;
				$(oEe.target.parentNode).css({
					'position':'absolute',
					'top':iTop+'px',
					'left':iLeft + 'px',
					'width':iW +'px',
				...
				function move (ev) {
					...
					var iNowT = oEe.target.parentNode.offsetTop;
