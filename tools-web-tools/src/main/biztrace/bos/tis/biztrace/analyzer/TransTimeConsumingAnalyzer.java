...
import bos.tis.biztrace.BizTraceLogRecord;
import bos.tis.biztrace.TransStepTimeConsuming;
...
public class TransTimeConsumingAnalyzer extends AbstractAnalyzer {
	...
	protected void doAnalyzed(String date, Jedis jedis) {
		...
		{
			logger.debug(Helper.concat("分析交易流水:",serialNo));
	...
	private TransTimeConsuming calculate(Map<byte[],byte[]> transBiztrace,String serialNo,Map<String,String> bizAnalyResult) {
		...
		while( i.hasNext() ){
			...
			stc.stepLog = b ; 
			stc.stepTimes = Helper.toTime(new String(e.getKey()).substring(0, 23)) ;//将步骤执行时间转换为毫秒数			
			stc.stepName = b.desc ;
			ttc.stepDtl.add(stc) ;
		}
