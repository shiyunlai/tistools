
/**
 * Copyright (C) 2016 bronsp.com, All rights reserved.
 */
package org.tis.tools.rservice.om.basic;

import java.util.List;

import org.tis.tools.base.WhereCondition;
import org.tis.tools.model.po.om.OmEmpPosition;

 
/**
 * <pre>
 * 员工岗位对应关系(OM_EMP_POSITION)的基础远程服务接口定义
 * </pre>
 *
 * @autor generated by tools:gen-dao
 *
 */
public interface IOmEmpPositionRService {
	
	/**
	 * 新增员工岗位对应关系(OM_EMP_POSITION)
	 * @param t 新记录
	 */
	public void insert(OmEmpPosition t);

	/**
	 * 更新员工岗位对应关系(OM_EMP_POSITION),只修改t对象有值的字段
	 * @param t 新值
	 */
	public void update(OmEmpPosition t);

	/**
	 * 强制更新员工岗位对应关系(OM_EMP_POSITION)
	 * @param t 新值
	 */
	public void updateForce(OmEmpPosition t);

	/**
	 * 删除员工岗位对应关系(OM_EMP_POSITION)
	 * @param guid 记录guid
	 */
	public void delete(String guid);

	/**
	 * 根据条件删除员工岗位对应关系(OM_EMP_POSITION)
	 * @param wc 条件
	 */
	public void deleteByCondition(WhereCondition wc);

	/**
	 * 根据条件更新员工岗位对应关系(OM_EMP_POSITION)
	 * @param wc 条件
	 * @param t 新值
	 */
	public void updateByCondition(WhereCondition wc, OmEmpPosition t);

	/**
	 * 根据条件查询员工岗位对应关系(OM_EMP_POSITION)
	 * @param wc 条件
	 * @return 满足条件的记录list
	 */
	public List<OmEmpPosition> query(WhereCondition wc);

	/**
	 * 根据条件统计员工岗位对应关系(OM_EMP_POSITION)记录数
	 * @param wc 条件
	 * @return 记录数
	 */
	public int count(WhereCondition wc);

	/**
	 * 根据id查询员工岗位对应关系(OM_EMP_POSITION)记录
	 * @param guid 记录guid
	 * @return 匹配的记录
	 */
	public OmEmpPosition loadByGuid(String guid);
}
